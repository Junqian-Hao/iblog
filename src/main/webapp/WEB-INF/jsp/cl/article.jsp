        ...
        <c:forEach items="${ArticleMap.Comments}" var="comment" varStatus="x">
            <c:if test="${x.count%2==0}">
                <li class="item cl"><a href="#"><i class="avatar size-L radius"><img alt=""
                                                                                     src="${pageContext.request.contextPath}/img/zbdxxh.png"></i></a>
                    <div class="comment-main" style="text-align: left">
                        <header class="comment-head">
                            <c:if test="${comment.user.uid==User.uid}">
                                <a href="/cl/deleteComment?cid=${comment.cid}&aid=${ArticleMap.Article.aid}"
                ...
                <li class="item cl comment-flip" style="text-align: right"><a href="#"><i
                    ...
                    <div class="comment-main" style="text-align: right">
                        <header class="comment-head">
                            <c:if test="${comment.user.uid==User.uid}">
                                <a href="/cl/deleteComment?cid=${comment.cid}&aid=${ArticleMap.Article.aid}"
                                   style="float: left;font-size:10px;margin-right: 30px">删除</a>
                            </c:if>
    ...
    <div id="comment">
        <form action="/cl/writeComment?aid=${ArticleMap.Article.aid}" method="post">
            <c:if test="${sessionScope.User!=null}">
                <input style="width:30%" class="form-control" name="w_nickname" value="${sessionScope.User.nickname}"
                       readonly>
            </c:if>
            <c:if test="${sessionScope.User==null}">
                <input style="width:30%;color: red" class="form-control" name="w_nickname" value="您尚未登录请登录" readonly>
            </c:if>
            <br/>
            ...
            <c:if test="${sessionScope.User==null}">
                <button disabled="disabled" class="btn btn-default">评论</button>
            </c:if>
            <br/>
        </form>
    </div>
